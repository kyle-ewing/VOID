...
### Map Information
- **GameMap** (`map/bwemwrappers/GameMap.java`) - Owns BWEM (the only place `bwem.*` is imported, alongside the other wrappers) and builds the wrapper graph once on game init. Wraps BWEM's `Base`/`Area`/`ChokePoint`/`Geyser`/`Mineral`/`StaticBuilding` with plain-named classes in `map/bwemwrappers/`. Splits areas at ground-height transitions, creates synthetic choke points on the divides, cuts overly-long snaking areas, then re-homes all chokes/bases/resources onto the resulting sub-areas. Exposes `getArea(tile)`, `getNearestArea`, `areaPath`, `chokePath`, and the per-tile walkable/height grids. All other code consumes wrappers, never BWEM directly.
- **MapInfo** (`information/MapInfo.java`, reached via `gameState.getBaseInfo()`) - Analyzes map structure on top of `GameMap`. Tracks base locations (starting, natural, expansions), choke points, siege positions, tile sets per base, and geyser locations. Initialized once in `onStart()` via `init()`. Disseminates map data to all managers.
